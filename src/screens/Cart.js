...
export default function Cart() {
  ...
  const handleCheckOut = async () => {
    ...
    console.log('Order Responde',response)
          ...
          <tbody>
            {data.map((food, index) => (
              <tr>
                ...
                <td>{food.size}</td>
                <td>{food.price}</td>
                <td ><button type="button" className="btn p-0">< img  style={{ width: '20px', height: '20px' }} src={del} onClick={() => { dispatch({ type: "REMOVE", index: index }) }} /></button> </td></tr>
            ))}
          </tbody>
        </table>
